...
double homeBaseLastSeenTime = {0.0};
...
void behaviourStateMachine(const ros::TimerEvent&)
{
  ...
  if (currentMode == 2 || currentMode == 3) {
      ...
      if (homeTagCount > 0){
          homeBaseLastSeenTime = currentTime;
      }else{
          homeBaseLastSeenTime = 0.0;
      }
      ...
      if (backUp) {
          if (currentTime > backUpStartTime + 7.2) {
              ...
              if ((sonarLeftRange < 0.8 || sonarRightRange < 0.8) && currentTime > homeBaseLastSeenTime + 93.93) {
                  obstacleStartTime = currentTime;
                  result.avoidingObstacle = true;
              } else {
