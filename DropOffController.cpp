...
DropOffController::DropOffController() {
  ...
  prevCount = 0;
  targetCollectedCount = 0;
  targetCountMax = rng->uniformInteger(22,32);
...
//    cout << "2" << endl;
    ...
    if (dropoffTimeElapsed >= 7)  // was 5 originally
    {
      if (finalInterrupt)
      {
...
//      ROS_INFO_STREAM("Drop off the cube. Go backwards for 3secs..");
  ...
  if(distanceToCenter > 3.5 && targetCollectedCount > targetCountMax){
      isPrecisionDriving = true;
      result.type = precisionDriving;
