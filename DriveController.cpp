...
//          ROS_INFO_STREAM("Obstacle waypoint added.");
        ...
        int size_local_waypoints = waypoints.size();
        int size_global_waypoints = result.wpts.waypoints.size();
        int local_back_id;
        int global_back_id;
        if (size_local_waypoints > 0) {local_back_id = waypoints.back().id;}else{local_back_id=0;}
        if (size_global_waypoints > 0) {global_back_id = result.wpts.waypoints.back().id;}else{local_back_id=0;}
//        ROS_INFO_STREAM(" Lsize:" << size_local_waypoints
//                       << " Gsize:" << size_global_waypoints
//                       << " Lid:" << local_back_id
//                       << " Gid:" << global_back_id   );
	if (size_local_waypoints < 0 || size_local_waypoints > 10000) waypoints.clear();
